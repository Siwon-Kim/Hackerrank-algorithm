...
TEST_CASE("Project 4 Test Cases for Dijkstra's Algorithm")
{
  ...
  SECTION("Should have the labels for the shortest path for graph 1")
  {
    ...
    std::vector<std::string> path;
    std::vector<std::string> correctPath;
    ...
    g.shortestPath("3", "6", path);
    correctPath = {"3", "6"};
    REQUIRE(path == correctPath);
    path.clear();
    correctPath.clear();

    g.shortestPath("5", "2", path);
    correctPath = {"5", "4", "2"};
    REQUIRE(path == correctPath);
    path.clear();
    correctPath.clear();
  ...
  {
    for (const auto label : vertices2)
      g.addVertex(label);
  ...
  SECTION("Should have the labels for the shortest path for graph 2")
  {
    ...
    g.shortestPath("ENB", "SUN", path);
    correctPath = {"ENB", "SUN"};
    REQUIRE(path == correctPath);
    path.clear();
    correctPath.clear();

    g.shortestPath("LIB", "CAS", path);
    correctPath = {"LIB", "ENB", "CAS"};
    REQUIRE(path == correctPath);
    path.clear();
    correctPath.clear();

    g.shortestPath("MSC", "SUB", path);
    correctPath = {"MSC", "LIB", "SUN", "SUB"};
    REQUIRE(path == correctPath);
    path.clear();
    correctPath.clear();
  }
  ...
  SECTION("Should have the labels for the shortest path for graph 3")
  {
    ...
    correctPath = {"1", "2", "3", "7", "12"};
    REQUIRE(path == correctPath);
    path.clear();
    correctPath.clear();

    g.shortestPath("3", "11", path);
    correctPath = {"3", "2", "6", "11"};
    REQUIRE(path == correctPath);
    path.clear();
    correctPath.clear();

    g.removeVertex("3");
    g.shortestPath("8", "7", path);
    correctPath = {"8", "9", "10", "11", "12", "7"};
    REQUIRE(path == correctPath);
    path.clear();
    correctPath.clear();
  }
